...
---
# Vendored into agent-runbook (source of truth). Provenance: vercel/turborepo
# skills/turborepo @ 2.10.13-canary.4, studied in prior-art/ (that copy is a frozen
# study snapshot). Update by re-vendoring here, then re-run install.sh — never edit
# the installed symlink target from prior-art.
...
```json
{
  "$schema": "https://v2-10-13-canary-4.turborepo.dev/schema.json",
  "tasks": {
    "build": {
      "dependsOn": ["^build"],
